...
import { ChevronDown, History, HardDrive, Play, Download, Trash, Edit, X, Zap, UploadCloud, FileVideo, Plus, Loader2, Cpu, Link as LinkIcon, Copy, Info } from 'lucide-react';
...
export default function App() {
  ...
  const showAlert = (title: string, message: string | React.ReactNode, variant: "danger" | "info" = "info") => {
    setModalConfig({
      isOpen: true,
      title,
      message,
  ...
  const formatOptions = [
    ...
    { value: "pcm", label: "PCM Raw (Uncompressed)" },
    { value: "flac", label: "FLAC (Lossless)" },
    { value: "alac", label: "ALAC (.m4a Lossless)" },
    { value: "ape", label: "APE (Monkey's Audio)" },
    { value: "wv", label: "WavPack (.wv)" },
    { value: "mp3", label: "MP3 (Lossy)" },
    { value: "aac", label: "AAC (.m4a Lossy)" },
    { value: "opus", label: "Opus (Lossy)" },
    { value: "ogg", label: "Ogg Vorbis (Lossy)" },
    { value: "wma", label: "WMA (Lossy)" }
  ];
    ...
    try {
      ...
      const assembleRes = await fetch('/api/assemble', {
        ...
        body: JSON.stringify({
          ...
          filename: file.name,
          options: { format, bitrate, channels, bit_depth: bitDepth }
        })
      });
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white/30 p-4 md:p-8 flex flex-col items-center relative overflow-hidden">
      ...
      {showFormatGuide && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl transition-all duration-300">
          <div className="bg-[#111111]/95 border border-white/10 rounded-[2rem] p-8 w-full max-w-2xl max-h-[85vh] shadow-2xl relative flex flex-col backdrop-blur-3xl">
            <button onClick={() => setShowFormatGuide(false)} className="absolute top-6 right-6 text-white/60 hover:text-white transition-colors bg-white/5 p-2 rounded-full z-10">
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-2xl font-semibold text-white mb-4">คู่มือรูปแบบไฟล์เสียง (Audio Formats)</h3>
            ...
            <div className="overflow-y-auto custom-scrollbar pr-4 space-y-6">
              ...
              <div>
                <h4 className="text-lg font-medium text-white mb-2 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-blue-400"></div> 1. ไม่บีบอัด (Uncompressed)
                </h4>
                <p className="text-sm text-white/70 mb-3 leading-relaxed">คุณภาพสูงสุด ไฟล์ใหญ่มาก (ต้นฉบับ)</p>
                <div className="grid gap-3">
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">WAV:</span> <span className="text-sm text-white/70">มาตรฐานสากล ใช้เยอะสุดในงานตัดต่อ/สตูดิโอ (Windows นิยม)</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">AIFF:</span> <span className="text-sm text-white/70">เหมือน WAV แต่พัฒนาโดย Apple (Mac นิยม)</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">PCM:</span> <span className="text-sm text-white/70">ข้อมูลดิบ 100% ไม่มี Header มักใช้เชิงเทคนิค/โค้ดดิ้ง</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="text-lg font-medium text-white mb-2 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-green-400"></div> 2. บีบอัดแบบไม่สูญเสีย (Lossless)
                </h4>
                <p className="text-sm text-white/70 mb-3 leading-relaxed">คุณภาพเท่าต้นฉบับเป๊ะ แต่ไฟล์เล็กลง 40-50% (ใช้เก็บ Archive)</p>
                <div className="grid gap-3">
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">FLAC:</span> <span className="text-sm text-white/70">ยอดฮิตที่สุดของ Lossless ไฟล์เล็ก เปิดได้เกือบทุกแอป</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">ALAC (.m4a):</span> <span className="text-sm text-white/70">พัฒนาโดย Apple ใช้ใน iTunes และระบบ iOS</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">APE:</span> <span className="text-sm text-white/70">บีบอัดได้เล็กที่สุด แต่กินพลังประมวลผลตอนเปิดมากกว่าตัวอื่น</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">WavPack (.wv):</span> <span className="text-sm text-white/70">ทางเลือกเฉพาะกลุ่ม รองรับไฟล์คุณภาพสูงบีบอัดผสม</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h4 className="text-lg font-medium text-white mb-2 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-yellow-400"></div> 3. บีบอัดแบบสูญเสียข้อมูล (Lossy)
                </h4>
                <p className="text-sm text-white/70 mb-3 leading-relaxed">ตัดเสียงที่คนไม่ได้ยินทิ้ง ไฟล์เล็กมาก เหมาะทำแอปสตรีมมิ่ง</p>
                <div className="grid gap-3">
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">MP3:</span> <span className="text-sm text-white/70">รองรับ 100% กับทุกอุปกรณ์บนโลก คุณภาพพอใช้ (มาตรฐาน)</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">AAC (.m4a):</span> <span className="text-sm text-white/70">ตัวสืบทอด MP3 ฉลาดกว่า คุณภาพเสียงดีกว่าที่ขนาดไฟล์เท่ากัน (YouTube/Spotify ใช้)</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">Opus:</span> <span className="text-sm text-white/70">กษัตริย์ในยุคใหม่ ดีที่สุดสำหรับการสตรีม ดีเลย์ต่ำมาก (Discord/WebRTC ใช้)</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">Ogg Vorbis:</span> <span className="text-sm text-white/70">ฟรี Open Source (Spotify เคยใช้) ประสิทธิภาพดีกว่า MP3</span>
                  </div>
                  <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                    <span className="font-bold text-white mr-2">WMA:</span> <span className="text-sm text-white/70">ของ Microsoft นิยมในอดีต (ไม่ค่อยแนะนำแล้วในปัจจุบัน)</span>
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-5 bg-[#111111]/60 backdrop-blur-3xl border border-white/10 p-8 rounded-[2.5rem] shadow-2xl flex flex-col gap-8 relative">
          ...
          <div className="grid grid-cols-2 gap-5 relative z-50">
            <div className="col-span-2 relative">
              <div className="flex items-center justify-between absolute -top-8 right-0 z-[100] mt-1 mr-1">
                 <button onClick={() => setShowFormatGuide(true)} className="text-white/50 hover:text-white transition-colors bg-white/5 hover:bg-white/10 p-1.5 rounded-full border border-white/5">
                    <Info className="w-4 h-4" />
                 </button>
              </div>
              <CustomSelect label="Format" value={format} options={formatOptions} onChange={(e: any) => setFormat(e.target.value)} />
            </div>
            <div>
                    ...
                    <div className="flex gap-2.5">
                      ...
                      <button onClick={() => {
                        ...
                        if (baseUrl.includes('localhost') || baseUrl.includes('127.0.0.1')) {
                          baseUrl = baseUrl.replace(/(localhost|127\.0\.0\.1)/, '192.168.1.10');
                        }
                        const link = `${baseUrl}/home/link/${item.token}`;
                        ...
                        const copyToClipboard = async (text: string) => {
                          try {
                            if (navigator.clipboard && window.isSecureContext) {
                              ...
                              // Fallback for non-secure contexts (like http://192.168.x.x on mobile)
                        ...
                        showAlert(
                          ...
                          <div className="mt-4 flex flex-col gap-3 w-full">
                            <p className="text-sm text-zinc-300">คุณสามารถคัดลอกและแชร์ลิงก์ด้านล่างเพื่อดาวน์โหลดไฟล์นี้ได้ทันที:</p>
                            <div className="flex z-50 items-center justify-between gap-3 w-full bg-black/40 border border-white/10 p-3 rounded-xl overflow-hidden relative group">
                              <input 
                                id={`link-input-${item.token}`}
                                type="text" 
                                readOnly 
                                value={link} 
                                className="bg-transparent border-none outline-none w-full text-sm font-mono text-emerald-400 selection:bg-emerald-900 overflow-hidden text-ellipsis whitespace-nowrap pr-24" 
                                onClick={(e) => {
                                  e.currentTarget.select();
                                  e.currentTarget.setSelectionRange(0, 99999); // For mobile devices
                                }} 
                              />
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center bg-[#151515] p-1 rounded-lg border border-white/10 shadow-lg">
                                <button 
                                  onClick={async () => {
                                    const success = await copyToClipboard(link);
                                    const btn = document.getElementById(`copy-btn-${item.token}`);
                                    if (btn) {
                                      const original = btn.innerHTML;
                                      btn.innerHTML = success ? "Copied!" : "Failed";
                                      btn.classList.add(success ? "text-emerald-400" : "text-red-400");
                                      setTimeout(() => {
                                  ...
                                  id={`copy-btn-${item.token}`} 
                                  className="px-3 py-1.5 hover:bg-white/10 text-white rounded-md text-xs font-semibold shrink-0 transition-colors flex items-center gap-1.5"
                                >
                            ...
                            <p className="text-xs text-zinc-500 mt-1 flex items-center gap-1.5">
                              ...
                              หากปุ่มคัดลอกไม่ทำงาน ให้แตะที่ลิงก์เพื่อคลุมดำและคัดลอกเอง
                            </p>
                            {item.token_created_at && (
                              <p className="text-[10px] text-zinc-600 font-mono mt-2 pt-2 border-t border-white/5">
                                GENERATED: {item.token_created_at}
                              </p>
                            )}
                          </div>,
                          "info"
                        );
                      }} className="px-3 py-2 bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white rounded-xl text-xs flex items-center justify-center transition-all" title="Copy Link">
